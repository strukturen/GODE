...
class LinearPointEmbedder(Embedder):
    ...
    def forward(self, sequences: List[Sequence], return_before_embed=False) -> Tuple[torch.Tensor, torch.Tensor]:
        #yuk (t_i, x_i) for each data point
        sequences = self.encode(sequences)
        #yuk (n_data x 21?)
        #print('2', sequences[0].shape)
        sequences, sequences_len = self.batch(sequences)
        #yuk (batch_size x 21?), 21 = float_descriptor_length?
        #print('3', sequences[0].shape)
        ...
        #yuk (batch_size x 21 x input_dim)
        #print('4', sequences_embeddings[0].shape)
        sequences_embeddings = self.compress(sequences_embeddings)
        #yuk (n_data x batch_size x input_out), n_data = sequences_len
        #print('5', sequences_embeddings.shape)
