...
class EnvDataset(Dataset):
        ...
        if path is not None:
            ...
            if params.batch_load and self.train:
                ...
                with h5py.File(path, 'r') as f:
                    lines = f['equations'][:]
                    trajectories.extend([f[f'matrices/matrix_{i}'][:] for i in range(len(f['matrices']))])
                data = []
                symplifier = simplifiers.Simplifier(generators.RandomFunctions(params, SPECIAL_WORDS))
                for i in tqdm(range(len(lines))): #len(lines)
                    curr_dict = {}
                    curr_dict['times'] = trajectories[i][:, 0]
                    curr_dict['trajectory'] = trajectories[i][:, 1] #only u
                    equation = str(lines[i])[2:-1].replace('u', 'x_0')
                    curr_dict['tree'] = symplifier.sympy_expr_to_tree(sympy.sympify(equation))
                    curr_dict['dimension'] = 1
                    if i == 0:
                        print('Curr_dict', curr_dict['tree'])
                    data.append(curr_dict)
                # self.data = [xy.split("=") for xy in lines]
                # self.data = [xy for xy in self.data if len(xy) == 3]
                self.data = data # until here
    ...
    def read_sample(self):
        ...
        #x["tree"] = self.env.equation_encoder.decode(x["tree"].split(",")) #ADAPTED
