# Applied example: Forced Damped Linearized Pendulum
# ======================================================================================================================
# Karin Yu, 2025
# ODE: m*u'' + c*u' + k*u = f*sin(w*t)
...
f = 2 #N, amplitude of external force
w = 1/2 #rad/s, frequency of external force

true_eq = str(m)+'*diff(diff(u,t),t)+'+str(c)+'*diff(u,t)+'+str(k)+'*u-'+str(f)+'*sin('+str(w)+'*t)'
force_term = str(f)+'*sin('+str(w)+'*t)'
...
data = np.genfromtxt(example_directory+'Pendulum_MATLAB.csv', delimiter=',')
...
print("Simulated MSE: ", np.mean((m*d2u_dt2_csv+c * du_dt_csv + k * u_csv - f*np.sin(w * t_csv))**2))
