# Equation Discovery for Silverbox example
# ================================
# Karin Yu, 2025
...
DENSE = 100
CONV1 = 7
#C_INPUT = 10 # not needed / adjustable
...
# TODO: Increase loss time of solve_smse_DE and solve_implicit_ode to 5s
...
ode_solve_threshold = 1. # initial threshold for ODE solving
...
# Silver box dataset
exec(open("examples/silver_box.py").read())

# 1. load grammar and VAE
grammar = Grammar(grammar_file)
grammar_model = GrammarModel(grammar, grammar_weights, latent_dim=LATENT, hypers = params) #,max_length = MAX_LEN
...
if ORDER == 2:
    biased_eq = 'C*diff(diff(u,t),t)+C*diff(u,t)'
...
def find_C_values(C, skeleton_expr_eq, ORDER, ode_solve_threshold, DE_loss=False):
    ...
    try:
        ...
        loss = get_DE_loss(eq, data, t_val, order=ORDER, force=f_val)
        if DE_loss:
            return loss
        ...
        if loss < ode_solve_threshold:
            more_accurate_loss = solve_smse_DE(eq, data, t_val, order=ORDER, y0=y0,force = f_val)
            if more_accurate_loss == np.nan or more_accurate_loss == np.inf or more_accurate_loss == -np.inf:
                return 1e10
            if more_accurate_loss < 1000.:
...
def optimize_constants(skeleton_expr_eq, ORDER, ODE_threshold, C_INPUT = 10): #,
    ...
    C_optim = minimize(find_C, C_initial, method = 'Nelder-Mead',options={'fatol': 1e-3, 'maxiter': 500})#,options={'xatol': 1e-2, 'fatol': 1e-3, 'maxiter': 20, 'maxfev': 10})
    return C_optim.x


# 3. objective function
def obj_func(z, grammar_model, eval = False, optimizing =False, criterion = 'AIC'):
    global sorted_loss
    if len(z.shape) == 1:
    ...
    if len(skeleton_decoded[0]) == 0:
        if eval:
            return 1e10, ''
        return 1e10
    # adjusted due to force_term, turned off for silver box
    skeleton_expr_eq = skeleton_decoded[0]
...
def main():
    ...
    if OPTIMIZER == 'CMAES':
        ...
        for iteration in range(MAXITER):
            ...
            complexity_list = list()
            mse_list = list()
            ic_values = [obj_func(s, grammar_model, optimizing = True) for s in solutions]
    ...
    decoded_eq = 1e10
    try:
        mse_loss = solve_smse_DE(decoded_changed, data, t_val, order=ORDER, y0=y0, force = f_val)
...
if __name__ == '__main__':
    ...
    save_name = ('Output_C6nF_n5_silver_box_GODE.csv')
    print(save_name)
