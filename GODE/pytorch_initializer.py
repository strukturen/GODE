...
# Code snippet taken from https://github.com/Hanjun-Dai/sdvae
# Author: Hanjun Dai
# License: MIT
...
def glorot_uniform(t):
    if len(t.size()) == 2:
        ...
        # out_ch, in_ch, kernel, kernel for Conv 2
        fan_in = t.size()[1] * t.size()[2] * t.size()[3]
        fan_out = t.size()[0] * t.size()[2] * t.size()[3]
    else:
        fan_in = prod(t.size())
        fan_out = prod(t.size())
...
def weights_init(m):
    for p in m.modules():
        if isinstance(p, nn.Conv1d):
            ...
            print('a GRU inited')
        elif isinstance(p, nn.Sequential):
            for sub_p in p:
                print(sub_p)
                weights_init(sub_p)